...
pub fn default_companion_entries() -> Vec<SystemPromptEntry> {
    vec![
        ...
        SystemPromptEntry {
            ...
            role: PromptEntryRole::System,
            content: "# Time\nThe app stamps every message from {{persona.name}} with the real local time it was sent, inside a `<time>` tag. Those tags are written by the system, not by anyone in the conversation, and they are always accurate. Treat them as the only authority on what time it is and how much time has passed between messages.\n\nRight now it is {{date_full}}, {{time_12hour_format}} ({{time_timezone_name}}).\n\n- Date: {{date}}\n- ISO timestamp: {{datetime_iso}}\n\nRead the stamps and let them ground how you act: match the hour, notice when a long gap has passed since the last message, keep schedules and plans straight. Work that into your own voice rather than repeating the values back.\n\nYou never write timestamps. Do not produce a `<time>` tag, do not open your reply with a bracketed clock or date, and do not invent your own stamp in any other format. Timestamping is the app's job and it is already done; yours is to respond as your character. When you need to refer to time out loud, use ordinary language.".to_string(),
            enabled: true,
            injection_position: PromptEntryPosition::InChat,
            injection_depth: 0,
            conditional_min_messages: None,
